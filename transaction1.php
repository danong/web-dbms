     ...
     <div class="container-fluid">
      <div class="row">
        ...
        <div class="col-sm9 col-sm-offset-3 col-md10 col-md-offset-2 main" id="maincon">
            <h1 class="page-header">Properties by Branch</h1>
            <?php
            $conn=oci_connect('dong','dan0ng01', '//dbserver.engr.scu.edu/db11g');
            $branchno = $_POST['branchID'];
            if($conn){
                echo "<h4> Displaying properties for Branch: $branchno</h4>";
                }
            else {
                $e = oci_error;
            ...
            $sql = "SELECT Rental_Property.rentalNo, Rental_Property.street, Rental_Property.city, Rental_Property.monthRent
                    FROM Rental_Property, Employee
                    WHERE rental_property.status = 'available'
                        AND rental_property.supervisorID = Employee.empID
			AND Employee.branchNo = '$branchno'";
            ...
            echo "<TR><TH>Rental Number</TH><TH>Street</TH><TH>City</TH><TH>Rent</TH>";
	    ...
	    // second SQL statement to get # of properties
            $sql2 = "SELECT count(*) AS NUMBER_OF_ROWS
		    FROM Rental_Property, Employee
                    WHERE rental_property.status = 'available'
                        AND rental_property.supervisorID = Employee.empID
			AND Employee.branchNo = '$branchno'";
	    ...
	    OCI_DEFINE_BY_NAME($sql_statement2, 'NUMBER_OF_ROWS', $number_of_rows);
            OCIExecute($sql_statement2);
	    OCI_FETCH($sql_statement2);

	    // output results
            echo "<BR><p>$number_of_rows properties found</p>";

            // free resources and close connection
            OCIFreeStatement($sql_statement2);
